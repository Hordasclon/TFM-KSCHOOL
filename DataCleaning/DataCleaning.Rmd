...
#AQIjpg<- chartr("/","\\", paste(proydir,imgpath,"AQI.jpg",sep=""))
...
# AQI index is de maximum index of analyzed component AQI = max( AQIPM2.5, AQIPM10, AQIO3, ...)
    ...
    aggdf1 <- df %>% select(State.Name, county.Name,Cat,Category,AQI, yearmm) %>% group_by(State.Name, county.Name, yearmm) %>%  summarise(Cat=max(Cat,na.rm=TRUE), Category=max(Category,na.rm=TRUE) ,AQI=max(AQI,na.rm=TRUE),aggnum=n())
 
    df1<-df %>% select(State.Name, county.Name,Cat,Category,AQI, yearmm,Defining.Parameter)
    
    df2 <- left_join(aggdf1,df1,by=c("State.Name", "county.Name","Cat","Category","AQI", "yearmm"))
    
    # erasing duplicated rows
...
#We go through all the USA states with a loop, reading all excel sheets, except Alabama, Florida, Idaho and New Hampshire which has no data:
