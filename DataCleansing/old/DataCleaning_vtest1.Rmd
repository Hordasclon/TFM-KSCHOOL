...
#install.packages("sf") .- Geospatial sf data of several countries
#install.packages("sp") .- Geospatial sp data of several countries by lon lat
...
#install.packages(gganimate) .- To animate maps
...
---
## Defining reading function to load and group Air Qaulity data for the 2017 year to 2020 year
---

```{r def_read_func, include=FALSE}
...
# Inside function grouping AQI data by USA State:
# Here we find max AQI index from analyzed components, AQI = max( AQIPM2.5, AQIPM10, AQIO3, ...)
...
# The end of reader and group function by years
```

----------------------------------------------------------------------------------
## Calling function to load and group Air Qaulity data for the 2017 year to 2020 year:
----------------------------------------------------------------------------------

```{r call_read_func, include=FALSE}
...
----------------------------------------------------------------------------------
## Merging with geographical data for USA Counties, from package("sp"), and saving as RData:
----------------------------------------------------------------------------------

```{r Usa_Geo_Data, include=FALSE}

#  Generating ID field to join by it with USA Counties data
aggtot$ID <- paste(tolower(aggtot$State.Name),tolower(aggtot$county.Name), sep=",", collapse=NULL)

#Taking geographical data for USA counties:
counties <- map_data("county")

#  Generating ID field to join by it with AQI data
counties$ID <- paste(tolower(counties$region),tolower(counties$subregion), sep=",", collapse=NULL)

# mergin AQI aggregated data with con counties data to includegeographical data

aggtotcounties <- merge(x = aggtot, y = counties, by = "ID", all.x = TRUE)

#Geting USA States data 

states <- map_data("state")

states$ID <- toTitleCase(states$region)

# Getting world ma

world <- map_data("world")

# Identyfing AQI values for each cathegory
ncateg<- c(1,2,3,4,5,6)
categ <- c("Good","Moderate","Unhealthy for Sensitive Groups","Unhealthy","Very Unhealthy","Hazardous")

#Defining colors per cathegory
colscateg <- c("1"="green","2"="yellow","3"="orange","4"="red","5"="darkred", "6"="black")

Air_Quality_Indexes <-aggtotcounties
...
#Getting a list for dates for available dates
...
##df <- no_nulos  %>% filter(yearmm==list[[i]])
 ...
 ggUsa <- ggplot(data = world) +
        geom_sf()+
        geom_sf(data = states) +
        geom_sf(data = Air_Quality_Indexes, aes(geometry=geom,fill = maxCat)) +
        ...
        transition_time(yearmm)
#aes(yearmm,maxAQI, color=county.Name,label=county.Name))+
 
 df <- Air_Quality_Indexes  %>% filter(yearmm <as.Date("2017-12-31", format= "%Y-%m-%d"))

 
ggUsa <- ggplot(data=world, aes(x = long, y = lat, group = group)) +
  geom_polygon(fill="lightgray", colour = "white")

 
ggUsa <-  ggplot(data = world) +
          geom_polygon( data = states, aes(x=long,y=lat,group=group)) +
          geom_polygon( data = Air_Quality_Indexes, aes(x=long,y=lat,group=group)) +
  scale_fill_manual(values=colscateg, name="AQI Categories",breaks=ncateg, labels=categ)+
  ggtitle(paste("Air Quality in USA Counties",list[1],sep=" "))+
        coord_sf(xlim = c(-130, -58), ylim = c(24.0, 50.1), expand = FALSE) +
        ggtitle(paste("Air Quality in USA Counties",list[i],sep=" "))+
        theme(axis.title.x=element_blank(),axis.title.y=element_blank())
...
#        theme(axis.title.x=element_blank(),axis.title.y=element_blank())+
#          scale_fill_manual(values=colscateg, name="AQI Categories",breaks=ncateg, labels=categ) + 
gg <- animate(ggUsa, height = 2, width = 3, units = "in", res = 100,fps = 20, duration = 15)
gg
#anim_save("ggUsa.gif", ggUsa)
  ##ggsave(paste(pngpath,"ggUsa",lista[i],".png", sep=""),
    ##     width = 8, height = 6, units = 'in', dpi=110, pointsize=10)
##}
```

Showing plots maps as an animation
...
```{r if (knitr:::is_latex_output()) { knitr::asis_output(./png/) } else {knitr::include_graphics("ggUsa.gif")}}
...
# Generating a function to loop all created maps for a moving image
...
image_write(imganimvisits, paste(pngpath,gif_file,sep=""))

giffvisits<-image_read(paste(pngpath,gif_file,sep=""))
```
...
```{r if (knitr:::is_latex_output()) { knitr::asis_output(./png/) } else {knitr::include_graphics("ggvisitsUsa.gif")}}}
## , eval=FALSE
giffvisits
```
