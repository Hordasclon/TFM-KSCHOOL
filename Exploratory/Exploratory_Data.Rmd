...
TotData <- TotData%>%mutate(MONTH= as.integer(format(Dates, "%m")))
...
AggTotData <- TotData %>% group_by(STATES,MONTH,statreg,statabb) %>% summarize(mnVisits = mean(All_discharges), sdVisits = sd(All_discharges), mdVisits=median(All_discharges),mnPop = mean(POPULATION), sdPop= sd(POPULATION), mdPop=median(POPULATION),nyears = n())

f_visits_graphs <- for (i in 1:12){ 
  
  PartMonthData<- AggTotData %>%select(STATES,MONTH,statreg,statabb,mnVisits,mnPop) %>%filter(MONTH== i)
...
# January graphs from 2017 to 2020


# Histograms to analyze most relevant States Hospital Visits data 

```{r Histograms, message=FALSE}
# Feature All_discharges (Hospital Visits with respiratory illness)
#utils::View(TotData)

AggTotData <- AggTotData%>%mutate(RateVisits= round((mnVisits/mnPop)*100,4))

 for (i in 1:12) {

    PartMonthData<- AggTotData %>%select(STATES,MONTH,statreg,statabb,mnVisits,mnPop,RateVisits) %>%filter(MONTH== i)

    histogr <-hist(PartMonthData$RateVisits, breaks=20, xlim=c(0,0.2), ylim=c(0,10) , col=rgb(1,0,0,0.5) , xlab="Hosp.Visits rate by Population" , ylab="count" , main=paste("Month ",month.name[i],sep="") )
    
   assign(paste("histogr_",i,sep=""), (histogr))
 }

```

# We can see that in general most data are in interval 0.03 and 0.1 rates
 ...
 for (i in 1:12) {
   ...
   PartMonthData<- AggTotData %>%select(STATES,MONTH,statreg,statabb,mnVisits,mnPop,RateVisits) %>%filter(MONTH== i)
    ...
    states_graph <-  ggplot(data=PartMonthData, aes(x=reorder(STATES,-RateVisits), y=RateVisits,fill=STATES)) +  
     ...
     scale_fill_manual(labels=StatesOrd,values =getPalette,name = "States Order by Hosp.Visits rate" )+
     ggtitle(paste("Usa States order byHosp.Visits vs Population rate at ",month.name[i],sep=""))+
